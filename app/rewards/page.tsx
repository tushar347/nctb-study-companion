...
export default function RewardsPage() {
  ...
  async function loadOverview() {
    ...
    try {
      ...
      const data = (await response.json()) as ApiResponse<Overview>;

      if (!response.ok || !data.success) {
        throw new Error(
          data.error ?? "Rewards could not be loaded.",
        );
      }
    ...
    try {
      const response = await fetch(
        "/api/rewards/redeem-item",
        {
          ...
          body: JSON.stringify({
            studentKey: getStoredStudentKey(),
            code,
          }),
        },
      );
      ...
      if (!response.ok || !data.success) {
        throw new Error(
          data.error ?? "Reward redemption failed.",
        );
      }

      setMessage(
        `${data.reward?.rewardItem.name ?? "Reward"} redeemed successfully.`,
      );
  ...
  async function updateGoal(targetPoints: number) {
    setActionCode(`GOAL:${targetPoints}`);
    setMessage("");
    setError("");
    ...
    try {
      ...
      if (!response.ok || !data.success) {
        throw new Error(
          data.error ?? "Daily goal update failed.",
        );
      }

      setMessage(`Daily goal changed to ${targetPoints} points.`);
      await loadOverview();
    } catch (requestError) {
      setError(
        requestError instanceof Error
          ...
          <>
            <div className="grid gap-5 md:grid-cols-3">
              <LiquidCard className="p-5">
                <WalletCards className="text-blue-700" size={27} />
                <p className="mt-4 text-xs font-black uppercase text-slate-500">
                  Available points
                </p>
                <p className="mt-1 text-4xl font-black">
                  {overview.wallet.learningPoints}
                </p>
                <p className="mt-2 text-sm font-semibold text-slate-500">
                  Lifetime earned: {overview.wallet.lifetimePointsEarned}
                </p>
              </LiquidCard>

              <LiquidCard className="p-5">
                <Target className="text-emerald-700" size={27} />
                <p className="mt-4 text-xs font-black uppercase text-slate-500">
                  Today&apos;s points
                </p>
                <p className="mt-1 text-3xl font-black">
                  {overview.goal.progress.pointsEarned} / {overview.goal.setting.targetPoints}
                </p>
                <div className="mt-4 h-3 overflow-hidden rounded-full bg-slate-200">
                  <div
                    ...
                    style={{
                      width: progressWidth(
                        overview.goal.progress.pointsEarned,
                        overview.goal.setting.targetPoints,
                      ),
                    }}
                  />
                </div>
              ...
              <LiquidCard className="p-5">
                <Flame className="text-orange-600" size={27} />
                <p className="mt-4 text-xs font-black uppercase text-slate-500">
                  Goal streak
                </p>
                <p className="mt-1 text-4xl font-black">
                  {overview.goal.streak}
                </p>
                <p className="mt-2 text-sm font-semibold text-slate-500">
              ...
              <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
                <div>
                  ...
                  <p className="mt-2 text-sm font-semibold text-slate-600">
                    Complete {overview.goal.setting.targetPoints} points and {overview.goal.setting.targetActivityTypes} different activity types today.
                  </p>
                  <p className="mt-2 text-sm font-black text-blue-700">
                    Activity types completed: {overview.goal.progress.distinctActivityTypes} / {overview.goal.setting.targetActivityTypes}
                  </p>
                </div>
                ...
                <div className="flex flex-wrap gap-2">
                  {[
                    ...
                    <button
                      key={value}
                      type="button"
                      onClick={() => void updateGoal(Number(value))}
                      disabled={actionCode === `GOAL:${value}`}
                      className={`rounded-2xl px-4 py-3 text-sm font-black ${
                        overview.goal.setting.targetPoints === Number(value)
              ...
              {overview.goal.progress.completedAt ? (
                ...
                <div className="mt-5 rounded-2xl bg-blue-50 p-4 text-sm font-semibold text-blue-900">
                  Earn {Math.max(
                    ...
                    overview.goal.setting.targetPoints -
                      overview.goal.progress.pointsEarned,
                  )} more point(s) and complete {Math.max(
                    0,
                    overview.goal.setting.targetActivityTypes -
                      overview.goal.progress.distinctActivityTypes,
            ...
            <section>
              <div className="flex items-center gap-3">
                <Sparkles className="text-orange-600" size={27} />
                <div>
                  <h2 className="text-2xl font-black">Reward Store</h2>
              ...
              <div className="mt-5 grid gap-5 md:grid-cols-2 xl:grid-cols-3">
                {overview.catalog.map((item) => (
                  ...
                  >
                    <Trophy className="text-amber-600" size={27} />
                    ...
                    <div className="mt-5 flex items-center justify-between gap-3">
                      ...
                      <button
                        type="button"
                        onClick={() => void redeem(item.code)}
                        disabled={
                          actionCode === item.code ||
                          overview.wallet.learningPoints < item.costPoints
                        }
            ...
            <LiquidCard className="p-6">
              <h2 className="text-2xl font-black">My Reward Inventory</h2>
